...
begin
    ...
    $display("----------------------------------------------");
    $display("FIFO Count       : %0d", DUT.U_FIFO.count);
    $display("Write Pointer    : %0d", DUT.U_FIFO.write_pointer);
    $display("Read Pointer     : %0d", DUT.U_FIFO.read_pointer);
    $display("FIFO Empty       : %b", DUT.fifo_empty);
    $display("----------------------------------------------");
    

        //==================================================
    // TEST 12: FIFO COVERAGE
    //==================================================

    $display("");
    $display("==============================================");
    $display("TEST 12: FIFO DIRECT COVERAGE");
    $display("==============================================");

    $display("Disabling FIFO read operation...");

    force DUT.fifo_read_enable = 1'b0;

    // Directly drive FIFO write inputs
    force DUT.U_FIFO.request_valid = 1'b0;
    force DUT.U_FIFO.request_packet_in = 32'd0;

    #20;

    $display("Filling FIFO with 16 requests...");

    repeat (16)
    begin
        @(negedge clk);

        force DUT.U_FIFO.request_packet_in = 32'h10000000;
        force DUT.U_FIFO.request_valid = 1'b1;
    ...
    $display("");
    $display("FIFO STATUS AFTER 16 REQUESTS");
    $display("----------------------------------------------");
    $display("FIFO Count       : %0d", DUT.U_FIFO.count);
    $display("Write Pointer    : %0d", DUT.U_FIFO.write_pointer);
    $display("Read Pointer     : %0d", DUT.U_FIFO.read_pointer);
    $display("FIFO Empty       : %b", DUT.fifo_empty);
    $display("FIFO Full        : %b", DUT.U_FIFO.fifo_full);
    $display("----------------------------------------------");

    //==================================================
    // Attempt 17th write while FIFO is full
    //==================================================

    $display("");
    $display("Attempting additional request when FIFO is full...");

    @(negedge clk);

    force DUT.U_FIFO.request_packet_in = 32'h17000000;
    force DUT.U_FIFO.request_valid = 1'b1;
    ...
    $display("");
    $display("FIFO STATUS AT FULL CONDITION");
    $display("----------------------------------------------");
    $display("FIFO Count       : %0d", DUT.U_FIFO.count);
    $display("Write Pointer    : %0d", DUT.U_FIFO.write_pointer);
    $display("Read Pointer     : %0d", DUT.U_FIFO.read_pointer);
    ...
    //==================================================
    // Enable FIFO read operation
    //==================================================

    $display("");
    $display("Re-enabling FIFO read operation...");

    release DUT.U_FIFO.request_valid;
    release DUT.U_FIFO.request_packet_in;

    force DUT.fifo_read_enable = 1'b1;
